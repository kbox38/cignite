...
async function createOrUpdateUserEnhanced(profileInfo, accessToken, dmaUrn, isDmaFlow) {
  ...
  try {
    ...
    // Multi-step user lookup logic (same as before)
    if (isDmaFlow && dmaUrn) {
    ...
    if (existingUser) {
      ...
      if (isDmaFlow) {
        ...
        const newUserData = {
          ...
          linkedin_dma_token: accessToken, // ← STORE DMA TOKEN FOR NEW USERS TOO!
